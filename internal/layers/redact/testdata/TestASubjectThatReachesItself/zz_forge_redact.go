...
// LogValue returns Cyclic as it may be logged.
//
...
func (v Cyclic) LogValue() slog.Value {
	nextLogged := slog.AnyValue(nil)
	if v.Next != nil {
		nextLogged = v.Next.LogValue()
	}

	return slog.GroupValue(
		slog.String("Token", "[redacted]"),
		slog.Attr{Key: "Next", Value: nextLogged},
	)
}
