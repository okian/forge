...
// patch has to name a type from.
const (
	modelPkg = "patchfixture/model"
	otherPkg = "patchfixture/other"
)
...
func loadFixture(t *testing.T) *load.Session {
	...
	dir, err := filepath.Abs(filepath.Join("testdata", "masks"))
	if err != nil {
...
// generating asks the layer for one fixture subject's patch and returns
...
func asking(t *testing.T, pkgPath, name string) (plugin.Unit, error) {
	...
	return patch.New().Generate(&plugin.Context{
		Model: &plugin.Model{
...
// written asks for a subject's patch and fails the test if the layer refused.
...
//
// Everything rather than the one unit about the subject, because a patch is
...
func source(t *testing.T, unit plugin.Unit) string {
	...
	out, err := file.Render()
	if err != nil {
		t.Fatalf("rendering the patch: %v", err)
	}
	return string(out)
}
...
func fixtureSource(t *testing.T) []byte {
	...
	return read(t, filepath.Join("testdata", "masks", "model", "model.go"))
}
...
func besideFixture(t *testing.T) goldentest.Package {
	...
	return goldentest.Package{
		Path:  otherPkg,
		Files: []goldentest.Source{{Name: "other.go", Content: read(t, filepath.Join("testdata", "masks", "other", "other.go"))}},
	}
}
