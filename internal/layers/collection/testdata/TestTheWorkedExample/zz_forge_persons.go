...
// were.
func (c Persons) SortedByID() []Person {
	return c.ordered(func(v Person) int {
		return v.ID
	})
}

// ByName returns the elements keyed by Name, keeping the last of any that share
// one.
func (c Persons) ByName() map[string]Person {
	return c.keyed(func(v Person) string {
		return v.Name
	})
}
