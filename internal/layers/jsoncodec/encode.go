...
func (w *writer) encoder(of *form) {
	...
	if of.attach {
		...
		w.line("func %s(%s []byte, %s %s) ([]byte, error) {", name, bufVar, valueVar, spelled)
		w.line("return %s.%s(%s)", valueVar, appendJSONMethod, bufVar)
		w.line("}")
		w.blank()

		w.line("// %s appends the %s to dst as a JSON object, and returns the", appendJSONMethod, spelled)
		...
		w.line("// growth of dst, which is what makes this the implementation the other")
		w.line("// entry points reach.")
		w.line("func (%s %s) %s(%s []byte) ([]byte, error) {", valueVar, spelled, appendJSONMethod, bufVar)
		w.appendBody(of)
		w.line("}")
		w.blank()
		...
		w.line("func (%s %s) %s() ([]byte, error) {", valueVar, spelled, marshalMethod)
		w.line("scratch := jsonTakeScratch()")
		w.line("held, err := %s.%s((*scratch)[:0])", valueVar, appendJSONMethod)
...
func (w *writer) appendBody(of *form) {
	...
	for i, one := range of.members {
		plans[i] = w.omitted(one, valueVar+"."+one.path)
		held := one.of
...
func (w *writer) writeMember(one member, held when, lead string) {
	...
	for _, guard := range one.guards {
		w.line("if %s.%s != nil {", valueVar, guard.path)
		closing++
	}
	...
	if held.retract {
		mark := loopVar("kept", w.marks)
		w.marks++
		w.line("%s := len(%s)", mark, bufVar)
		w.line("%s = append(%s, %s...)", bufVar, bufVar, goString(prefix))
		w.appendValue(valueVar+"."+one.path, &one.of, 0)
		w.line("if jsonWroteEmpty(%s, %s+%d) {", bufVar, mark, len(prefix))
		w.line("%s = %s[:%s]", bufVar, bufVar, mark)
		w.line("}")
	} else {
		w.line("%s = append(%s, %s...)", bufVar, bufVar, goString(prefix))
		w.appendValue(valueVar+"."+one.path, &one.of, 0)
...
func (w *writer) appendValue(held string, of *form, depth int) {
	...
	case writtenStruct:
		w.line("if %s, err = %s(%s, %s); err != nil {", bufVar, encoderFor(of.typ), bufVar, held)
		w.line("return %s, err", bufVar)
		w.line("}")

	case writtenPointer:
		w.line("if %s == nil {", held)
		w.line(`%s = append(%s, "null"...)`, bufVar, bufVar)
		w.line("} else {")
...
func (w *writer) appendScalar(held string, of *form, depth int) bool {
	switch of.how {
	case writtenBool:
		w.line("%s = strconv.AppendBool(%s, bool(%s))", bufVar, bufVar, held)

	case writtenInt:
		w.line("%s = strconv.AppendInt(%s, int64(%s), 10)", bufVar, bufVar, held)

	case writtenUint:
		w.line("%s = strconv.AppendUint(%s, uint64(%s), 10)", bufVar, bufVar, held)
	...
	case writtenFloat:
		...
		w.line("if %s, err = jsonAppendFinite(%s, float64(%s), %d); err != nil {", bufVar, bufVar, held, bits)
		w.line("return %s, err", bufVar)
		w.line("}")

	case writtenString:
		w.line("if %s, err = jsonAppendString(%s, string(%s)); err != nil {", bufVar, bufVar, held)
		w.line("return %s, err", bufVar)
		w.line("}")
	...
	case writtenBytes:
		...
		if _, fixed := of.typ.Underlying().(*types.Array); fixed {
			...
			w.line("{")
			w.line("%s := %s", one, held)
			w.line("%s = jsonAppendBytes(%s, %s[:])", bufVar, bufVar, one)
			w.line("}")
			return true
		}
		w.line("%s = jsonAppendBytes(%s, %s)", bufVar, bufVar, held)
...
func (w *writer) appendSpliced(held string, depth int) {
	spliced := loopVar("spliced", depth)
	failed := loopVar("failed", depth)

	w.line("{")
	w.line("%s, %s := json.Marshal(%s, json.Deterministic(true))", spliced, failed, held)
	w.line("if %s != nil {", failed)
	w.line("return %s, %s", bufVar, failed)
	w.line("}")
	w.line("%s = append(%s, %s...)", bufVar, bufVar, spliced)
	w.line("}")
}
...
func (w *writer) appendDelegate(held string, of *form, depth int) {
	...
	if valueMethod(of.typ, appendJSONMethod) {
		w.line("if %s, err = %s.%s(%s); err != nil {", bufVar, held, appendJSONMethod, bufVar)
		w.line("return %s, err", bufVar)
		w.line("}")
		return
	}
	...
	w.line("{")
	w.line("%s := %s", one, held)
	w.line("if %s, err = %s.%s(%s); err != nil {", bufVar, one, appendJSONMethod, bufVar)
	w.line("return %s, err", bufVar)
	w.line("}")
	w.line("}")
}
...
func (w *writer) appendText(held string, of *form, depth int) {
	...
	w.line("return %s, %s", bufVar, failed)
	w.line("}")
	w.line("if %s, err = jsonAppendString(%s, string(%s)); err != nil {", bufVar, bufVar, text)
	w.line("return %s, err", bufVar)
	w.line("}")
	w.line("}")
}
...
func (w *writer) appendArray(held string, of *form, depth int) {
	...
	w.line("}")
	w.line("if len(%s) == %s {", bufVar, mark)
	w.line("%s = append(%s, '[', ']')", bufVar, bufVar)
	w.line("} else {")
	w.line("%s[%s] = '['", bufVar, mark)
	w.line("%s = append(%s, ']')", bufVar, bufVar)
	w.line("}")
	w.line("}")
}
...
func (w *writer) appendMap(held string, of *form, depth int) {
	...
	w.line("%s = append(%s, ',')", bufVar, bufVar)
	w.line("if %s, err = jsonAppendString(%s, %s); err != nil {", bufVar, bufVar, key)
	w.line("return %s, err", bufVar)
	w.line("}")
	w.line("%s = append(%s, ':')", bufVar, bufVar)
