...
func (v Grade) String() string {
	...
	return "Grade(" + strconv.Quote(string(v)) + ")"
}
...
// ValuesGrade returns every member, in the order they were declared.
//
...
// ParseGrade returns the member with this name.
//
...
func ParseGrade(s string) (Grade, error) {
	...
	case "fail":
		return GradeFail, nil
	}

	var zero Grade
	return zero, errors.New(strconv.Quote(s) + " is not a member of Grade")
}
...
func (v Grade) MarshalText() ([]byte, error) {
	if !v.Valid() {
		return nil, errors.New(v.String() + " is not a member of Grade")
	}
...
// to call.
func (v Grade) AppendText(b []byte) ([]byte, error) {
	if !v.Valid() {
		return nil, errors.New(v.String() + " is not a member of Grade")
	}
...
func (v *Grade) UnmarshalText(b []byte) error {
	held, err := ParseGrade(string(b))
	if err != nil {
		return err
	}
