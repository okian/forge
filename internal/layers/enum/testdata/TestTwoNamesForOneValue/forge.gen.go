...
func (v Aliased) String() string {
	...
	return "Aliased(" + strconv.FormatInt(int64(v), 10) + ")"
}
...
// ValuesAliased returns every member, in the order they were declared.
//
...
// ParseAliased returns the member with this name.
//
...
// one was added.
func ParseAliased(s string) (Aliased, error) {
	switch s {
	case "first":
		return AliasedFirst, nil
	case "second":
		return AliasedSecond, nil
	case "one":
		return AliasedOne, nil
	}

	var zero Aliased
	return zero, errors.New(strconv.Quote(s) + " is not a member of Aliased")
}
...
func (v Aliased) MarshalText() ([]byte, error) {
	if !v.Valid() {
		return nil, errors.New(v.String() + " is not a member of Aliased")
	}
...
// to call.
func (v Aliased) AppendText(b []byte) ([]byte, error) {
	if !v.Valid() {
		return nil, errors.New(v.String() + " is not a member of Aliased")
	}
...
func (v *Aliased) UnmarshalText(b []byte) error {
	held, err := ParseAliased(string(b))
	if err != nil {
		return err
	}
