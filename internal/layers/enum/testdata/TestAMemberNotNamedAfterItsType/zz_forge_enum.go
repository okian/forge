...
func (v Loose) String() string {
	...
	return "Loose(" + strconv.FormatInt(int64(v), 10) + ")"
}
...
// ValuesLoose returns every member, in the order they were declared.
//
...
// ParseLoose returns the member with this name.
//
...
// one was added.
func ParseLoose(s string) (Loose, error) {
	switch s {
	case "first":
		return LooseFirst, nil
	case "Wandering":
		return Wandering, nil
	}

	var zero Loose
	return zero, errors.New(strconv.Quote(s) + " is not a member of Loose")
}
...
func (v Loose) MarshalText() ([]byte, error) {
	if !v.Valid() {
		return nil, errors.New(v.String() + " is not a member of Loose")
	}
...
// to call.
func (v Loose) AppendText(b []byte) ([]byte, error) {
	if !v.Valid() {
		return nil, errors.New(v.String() + " is not a member of Loose")
	}
...
func (v *Loose) UnmarshalText(b []byte) error {
	held, err := ParseLoose(string(b))
	if err != nil {
		return err
	}
