...
func (v Owned) String() string {
	...
	return "Owned(" + strconv.FormatInt(int64(v), 10) + ")"
}

// ValuesOwned returns every member, in the order they were declared.
//
...
// ParseOwned returns the member with this name.
//
...
// one was added.
func ParseOwned(s string) (Owned, error) {
	switch s {
	case "one":
		return OwnedOne, nil
	case "two":
	...
	var zero Owned
	return zero, errors.New(strconv.Quote(s) + " is not a member of Owned")
}
...
func (v Owned) MarshalText() ([]byte, error) {
	if !v.Valid() {
		return nil, errors.New(v.String() + " is not a member of Owned")
	}
...
// to call.
func (v Owned) AppendText(b []byte) ([]byte, error) {
	if !v.Valid() {
		return nil, errors.New(v.String() + " is not a member of Owned")
	}
...
func (v *Owned) UnmarshalText(b []byte) error {
	held, err := ParseOwned(string(b))
	if err != nil {
		return err
	}
