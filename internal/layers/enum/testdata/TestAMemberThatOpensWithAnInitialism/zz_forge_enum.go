...
func (v Coded) String() string {
	...
	return "Coded(" + strconv.FormatInt(int64(v), 10) + ")"
}
...
// ValuesCoded returns every member, in the order they were declared.
//
...
// ParseCoded returns the member with this name.
//
...
// one was added.
func ParseCoded(s string) (Coded, error) {
	switch s {
	case "ok":
		return CodedOK, nil
	case "httpError":
		return CodedHTTPError, nil
	case "id":
		return CodedID, nil
	}

	var zero Coded
	return zero, errors.New(strconv.Quote(s) + " is not a member of Coded")
}
...
func (v Coded) MarshalText() ([]byte, error) {
	if !v.Valid() {
		return nil, errors.New(v.String() + " is not a member of Coded")
	}
...
// to call.
func (v Coded) AppendText(b []byte) ([]byte, error) {
	if !v.Valid() {
		return nil, errors.New(v.String() + " is not a member of Coded")
	}
...
func (v *Coded) UnmarshalText(b []byte) error {
	held, err := ParseCoded(string(b))
	if err != nil {
		return err
	}
