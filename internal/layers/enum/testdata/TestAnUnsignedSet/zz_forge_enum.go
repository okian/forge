...
func (v Permitted) String() string {
	switch v {
	case PermittedNone:
		return "none"
	case PermittedAll:
	...
	return "Permitted(" + strconv.FormatUint(uint64(v), 10) + ")"
}
...
// ValuesPermitted returns every member, in the order they were declared.
//
...
func ValuesPermitted() []Permitted {
	return []Permitted{
		PermittedNone,
		PermittedAll,
	}
...
// ParsePermitted returns the member with this name.
//
...
// one was added.
func ParsePermitted(s string) (Permitted, error) {
	switch s {
	case "none":
		return PermittedNone, nil
	case "all":
		return PermittedAll, nil
	}

	var zero Permitted
	return zero, errors.New(strconv.Quote(s) + " is not a member of Permitted")
}
...
func (v Permitted) MarshalText() ([]byte, error) {
	if !v.Valid() {
		return nil, errors.New(v.String() + " is not a member of Permitted")
	}
...
// to call.
func (v Permitted) AppendText(b []byte) ([]byte, error) {
	if !v.Valid() {
		return nil, errors.New(v.String() + " is not a member of Permitted")
	}
...
func (v *Permitted) UnmarshalText(b []byte) error {
	held, err := ParsePermitted(string(b))
	if err != nil {
		return err
	}
