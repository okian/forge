...
func (v Renamed) String() string {
	switch v {
	case RenamedPass:
		return "pass"
	}
	return "Renamed(" + strconv.Quote(string(v)) + ")"
}
...
// ValuesRenamed returns every member, in the order they were declared.
//
...
func ValuesRenamed() []Renamed {
	return []Renamed{
		RenamedPass,
	}
}

// ParseRenamed returns the member with this name.
//
...
// one was added.
func ParseRenamed(s string) (Renamed, error) {
	switch s {
	case "pass":
		return RenamedPass, nil
	}

	var zero Renamed
	return zero, errors.New(strconv.Quote(s) + " is not a member of Renamed")
}
...
func (v Renamed) MarshalText() ([]byte, error) {
	if !v.Valid() {
		return nil, errors.New(v.String() + " is not a member of Renamed")
	}
...
// to call.
func (v Renamed) AppendText(b []byte) ([]byte, error) {
	if !v.Valid() {
		return nil, errors.New(v.String() + " is not a member of Renamed")
	}
...
func (v *Renamed) UnmarshalText(b []byte) error {
	held, err := ParseRenamed(string(b))
	if err != nil {
		return err
	}
