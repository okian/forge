...
// that finding one by Joined is a map access rather than a scan. The walk order
...
// ByJoined returns a pointer to the element held under this Joined, and whether
// one is.
func (r *Directory) ByJoined(k time.Time) (*Person, bool) {
	return r.pick(r.byJoined, k)
}
...
func (r *Directory) Remove(k time.Time) bool {
	e, held := r.byJoined[k]
	if !held {
		return false
	}
...
func (r *Directory) placeChecked(v Person) error {
	if _, held := r.byJoined[v.Joined]; held {
		return ErrDirectoryDuplicate
	}
	e := &directoryEntry{elem: v, at: len(r.order)}
	r.order = append(r.order, e)
	r.byJoined = r.noted(r.byJoined, v.Joined, e)
	return nil
}
