...
// place adds one element to the order and files it under every key, replacing
// the element its key already reaches — in place, so the entry stays where
// every lookup filed it.
func (r *Directory) place(v Person) {
	if e, held := r.byID[v.ID]; held {
