...
// instead.
func (g *GuardedPersons) AppendJSON(dst []byte) ([]byte, error) {
	held := g.Snapshot()
...
func (g *GuardedPersons) MarshalJSON() ([]byte, error) {
...
// GuardedPersonsEncodeLocked is Person behind a read-write lock.
//
// Everything the stack below offers is on guardedPersonsEncodeLockedHeld, which this holds
// and nothing else can reach. What reaches it is a scope: Do under the
// write lock, RDo under the read lock, each handed a GuardedPersonsEncodeLockedView for
// as long as the call lasts.
//
// The zero value holds a container that was never made and can hold
// nothing, so use NewGuardedPersonsEncodeLocked.
//
// The lock must not be copied once it has been used, which is why every
// method here is on the pointer.
type GuardedPersonsEncodeLocked struct {
	mu   sync.RWMutex
	held guardedPersonsEncodeLockedHeld
}

// NewGuardedPersonsEncodeLocked returns a lock around a new guardedPersonsEncodeLockedHeld.
//
// The container beneath a lock is made rather than declared — it has to be
// told how much it holds — so this is the way in, and the zero value of
// this type is a lock around a container that will refuse every element.
...
// What is made here is reachable through this lock and through nothing
// else, which is the whole of what the lock is for.
func NewGuardedPersonsEncodeLocked() *GuardedPersonsEncodeLocked {
	return &GuardedPersonsEncodeLocked{held: *newGuardedPersonsEncodeLockedHeld()}
}
...
func (g *GuardedPersonsEncodeLocked) Do(f func(v GuardedPersonsEncodeLockedView)) {
	...
	f(GuardedPersonsEncodeLockedView{held: &g.held})
}
...
// doubling and copy what it had each time — which is what collecting a
// sequence of unknown length has to do, and is not what this is.
func (g *GuardedPersonsEncodeLocked) Snapshot() []Person {
	g.mu.RLock()
	defer g.mu.RUnlock()
...
// It is a fact about the past by the time it is read, like every count of
// something another goroutine may be changing.
func (g *GuardedPersonsEncodeLocked) Len() int {
	g.mu.RLock()
	defer g.mu.RUnlock()
...
// AppendJSON appends the container to dst as a JSON array, with the
// read lock held for the length of the write.
//
// Written this way because the declaration asked for it, and it is the
// right answer only when nothing slow happens to the buffer meanwhile:
// nothing is copied, and every writer of this container waits for however
// long the append takes to finish.
func (g *GuardedPersonsEncodeLocked) AppendJSON(dst []byte) ([]byte, error) {
	g.mu.RLock()
	defer g.mu.RUnlock()
...
// holds a buffer appends with AppendJSON instead, which is the
// same document without the allocation.
func (g *GuardedPersonsEncodeLocked) MarshalJSON() ([]byte, error) {
	return g.AppendJSON(nil)
}

// GuardedPersonsEncodeLockedView is what a scope over GuardedPersonsEncodeLocked hands the function it runs.
//
// No method on it names this type or the one it guards, so the call that
// deadlocks by accident — reaching for the value you were just handed and
// finding a way back in — cannot be written through this.
...
//
// It is the same method the stack below declares, reached through the view.
func (v GuardedPersonsEncodeLockedView) Cap() int {
	return v.held.Cap()
}
...
//
// It is the same method the stack below declares, reached through the view.
func (v GuardedPersonsEncodeLockedView) Len() int {
	return v.held.Len()
}
...
//
// It is the same method the stack below declares, reached through the view.
func (v GuardedPersonsEncodeLockedView) All() iter.Seq[Person] {
	return v.held.All()
}
...
//
// It is the same method the stack below declares, reached through the view.
func (v GuardedPersonsEncodeLockedView) Backward() iter.Seq[Person] {
	return v.held.Backward()
}
...
//
// It is the same method the stack below declares, reached through the view.
func (v GuardedPersonsEncodeLockedView) Reset() {
	v.held.Reset()
}
...
//
// It is the same method the stack below declares, reached through the view.
func (v GuardedPersonsEncodeLockedView) Push(a0 Person) {
	v.held.Push(a0)
}
...
//
// It is the same method the stack below declares, reached through the view.
func (v GuardedPersonsEncodeLockedView) AppendSeq(a0 iter.Seq[Person]) {
	v.held.AppendSeq(a0)
}

// guardedPersonsEncodeLockedHeldFixedCap is how many elements the buffer holds.
//
// It is a constant rather than a field, so the size is part of the type rather
// than of a value: every container of this type holds the same number, the
// compiler knows it, and no caller can be handed one sized differently from the
...
// guardedPersonsEncodeLockedHeld holds a fixed number of the most recent elements.
//
// The buffer is allocated once and never grows, so a producer that outruns its
// consumer costs a bounded amount of memory rather than an increasing one. That
// is the whole of what this type is for, and it is why every method that adds
...
// newGuardedPersonsEncodeLockedHeld returns an empty container, whose capacity is the one the
// declaration fixed.
func newGuardedPersonsEncodeLockedHeld() *guardedPersonsEncodeLockedHeld {
	return &guardedPersonsEncodeLockedHeld{buf: make([]Person, guardedPersonsEncodeLockedHeldFixedCap)}
}

// Cap reports how many elements the container can hold, which does not change.
func (r *guardedPersonsEncodeLockedHeld) Cap() int { return len(r.buf) }

// Len reports how many elements the container holds, which is never more than
// its capacity.
func (r *guardedPersonsEncodeLockedHeld) Len() int { return r.n }
...
// are the ones kept, and how much memory that costs was decided when the
// container was made.
func (r *guardedPersonsEncodeLockedHeld) Push(v Person) {
	r.built()

	if r.n == len(r.buf) {
		r.buf[r.head] = v
		r.head = guardedPersonsEncodeLockedHeldIndexOf(r.head, 1, len(r.buf))
		return
	}

	r.buf[guardedPersonsEncodeLockedHeldIndexOf(r.head, r.n, len(r.buf))] = v
	r.n++
}
...
// container that exists to bound its memory should not do behind a caller's
// back. Walk it, or push to it, or take a copy and do both.
func (r *guardedPersonsEncodeLockedHeld) All() iter.Seq[Person] {
	held, from, size := r.n, r.head, len(r.buf)

	return func(yield func(Person) bool) {
		for i := range held {
			if !yield(r.buf[guardedPersonsEncodeLockedHeldIndexOf(from, i, size)]) {
				return
			}
		}
	}
...
// Backward walks the container from the newest element to the oldest. Like All,
// which slots it covers is fixed when it is called.
func (r *guardedPersonsEncodeLockedHeld) Backward() iter.Seq[Person] {
	held, from, size := r.n, r.head, len(r.buf)
...
// A sequence longer than the container leaves the last capacity elements of it,
// which is what pushing them one at a time would leave.
func (r *guardedPersonsEncodeLockedHeld) AppendSeq(seq iter.Seq[Person]) {
	for v := range seq {
		r.Push(v)
	}
}
...
// then. Nothing can read them: what a walk covers is the count, and the count
// is what this sets to none.
func (r *guardedPersonsEncodeLockedHeld) Reset() { r.head, r.n = 0, 0 }
...
// that as back-pressure and retries for ever. Saying the same thing both ways
// makes the mistake one answer rather than two.
func (r *guardedPersonsEncodeLockedHeld) built() {
	if len(r.buf) == 0 {
		panic("forge: this container was never constructed; use the constructor")
	}
}

// guardedPersonsEncodeLockedHeldIndexOf returns where the element i places after the one at from is kept.
//
// Given the buffer's own head and size it answers for the container as it is;
// given a head a walk took when it started, it answers for the container as it
// was, which is what keeps a walk on the positions it set out to visit.
...
// elements held, which is never more than the size, so the sum passes the end
// of the buffer at most once.
func guardedPersonsEncodeLockedHeldIndexOf(from, i, size int) int {
	if slot := from + i; slot < size {
		return slot
	}
	return from + i - size
...
// GuardedPersonsEncodeLocked satisfies these.
//
// The claim is checked when the package is built rather than when a caller
// first tries, so a stack that stops satisfying one of these fails here
// rather than at somebody's call site. And a reader who is not going to read
...
// writer and would rather not pay it can ask for the lock to be held
// instead.
func (g *GuardedPersonsExposeLocker) AppendJSON(dst []byte) ([]byte, error) {
	held := g.Snapshot()
...
// holds a buffer appends with AppendJSON instead, which is the
// same document without the allocation.
func (g *GuardedPersonsExposeLocker) MarshalJSON() ([]byte, error) {
	return g.AppendJSON(nil)
}
...
// appendMatrixPersonJSON appends a Person as JSON.
//
...
func appendMatrixPersonJSON(dst []byte, v Person) ([]byte, error) {
...
func (v *Person) UnmarshalJSON(data []byte) error {
	held := *v
	next, err := decodeMatrixPersonJSON(data, 0, 0, &held, false)
	if err != nil {
		return err
	}
...
func (v *Person) UnmarshalJSONBorrowed(data []byte) error {
	held := *v
	next, err := decodeMatrixPersonJSON(data, 0, 0, &held, true)
	if err != nil {
		return err
	}
	if err := jsonAtEnd(data, next); err != nil {
...
// decodeMatrixPersonJSON reads one Person from b at i, and returns where the next value
// begins.
//
