...
func (g *GuardedPersonsEncodeLocked) MarshalJSONTo(enc *jsontext.Encoder) error {
	g.mu.RLock()
	defer g.mu.RUnlock()
...
func encodeMatrixPersonJSONTo(enc *jsontext.Encoder, v Person) error {
...
// decodeMatrixPersonJSONFrom reads a Person from JSON.
//
// The value's own method holds the body; this is what generated code
// calls, so that a caller names one function whether or not the type
// is one a method could be declared on.
func decodeMatrixPersonJSONFrom(dec *jsontext.Decoder, v *Person) error {
