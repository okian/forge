...
func TestADeclarationThatChangedSinceItsFileWasWritten(t *testing.T) {
	...
	if !strings.Contains(said, "FRG5004") || !strings.Contains(said, "forge.gen.go") {
...
func TestADeclarationWhoseFileIsNotThere(t *testing.T) {
	...
	if !strings.Contains(said, "FRG5005") || !strings.Contains(said, "forge.gen.go") {
...
func TestAFileForgeWillNotWriteOver(t *testing.T) {
	...
	for _, want := range []string{"FRG5006", "does not say forge wrote it", "move it out of the way", "delete it"} {
...
// A file nothing in the package asks for is reported by the same run, because
// an author asking whether their tree is current wants both answers at once.
func TestAFileNoDeclarationAccountsFor(t *testing.T) {
	...
	left := filepath.Join(dir, "forge_stubs.gen.go")
...
// How a file is out of date, told apart from merely having been written by
// something else.
//
// Two very different things move a fingerprint: a declaration somebody edited,
// and a forge somebody upgraded. Only the first means the file no longer
// describes the source beside it, and the second is the ordinary state of a
// working directory — so a report that could not tell them apart would be one
// people learn to ignore, which is the same as not having it.
func TestHowAFileIsOutOfDate(t *testing.T) {
	cfg := generated.Config{Forge: "v2", Markers: "m2", Toolchain: "go1.27.0"}

	// Assembled the way the pipeline assembles one, so that what is
	// fingerprinted is a declaration rather than a value put together here.
	pkg := grouped(generating(t, t.TempDir(), "Persons").modelled)[0]
	held := pkg.requests[0]

	// What this run would record, which is the fingerprint a current file has.
	var sum emit.Digest
	generated.FingerprintPackage(&sum, []generated.Request{held}, "model", cfg)

	// And what a run by different tooling would have recorded for the same
	// declaration, which is the case worth telling apart.
	older := generated.Config{Forge: "v1", Markers: "m1", Toolchain: "go1.27.0"}

	var was emit.Digest
	generated.FingerprintPackage(&was, []generated.Request{held}, "model", older)
	...
	cases := map[string]struct {
		...
		"current": {
			recorded: emit.Header{Forge: "v2", Markers: "m2", Toolchain: "go1.27.0", Inputs: sum.String()},
		},

		"written by an older forge": {
			recorded: emit.Header{Forge: "v1", Markers: "m1", Toolchain: "go1.27.0", Inputs: was.String()},
			says:     "unchanged, and was written by forge v1 rather than v2, markers m1 rather than m2",
			moved:    true,
		},

		"the declaration changed": {
			recorded: emit.Header{Forge: "v2", Markers: "m2", Toolchain: "go1.27.0", Inputs: "0000000000000000"},
		...
		// writes. Passing it over would turn the check off for that file for
		// good, since nothing else would ask again.
		"no fingerprint": {
			recorded: emit.Header{Forge: "v2", Markers: "m2"},
	...
	for name, want := range cases {
		t.Run(name, func(t *testing.T) {
			got, moved := staleness(want.recorded, []generated.Request{held}, "model", cfg)
			...
			if moved != want.moved {
				t.Errorf("moved=%v, want %v: %q", moved, want.moved, got)
			}
		})
	}
}

// A file written by different tooling is reported under its own code, so that
// a reader can tell it from one whose declaration has changed.
func TestAFileWrittenByDifferentTooling(t *testing.T) {
	dir := t.TempDir()
	checkable(t, dir)

	// Generated by this build and then checked by one calling itself something
	// else, which is the state a file committed by a colleague is in.
	stamped(t, "github.com/okian/forge", "v0.0.1-somebody-else")

	_, said, err := checking(t, generating(t, dir, "Persons"), "./...")
	if err == nil {
		t.Fatal("a file written by different tooling was passed")
	}

	if !strings.Contains(said, "FRG5007") {
		t.Errorf("it was not reported under its own code:\n%s", said)
	}
	if strings.Contains(said, "FRG5004") {
		t.Errorf("it was reported as though the declaration had changed:\n%s", said)
	}
	if !strings.Contains(said, "nothing about the declarations has changed") {
		t.Errorf("the report does not say what is and is not wrong:\n%s", said)
	}
}
