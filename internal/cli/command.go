...
type environment struct {
	...
	// catalog is the layers this run knows about.
	//
	// Held rather than reached for, because which layers exist is the one thing
	// about a run that does not come off the command line: a layer is code, so
	// a binary that knows about one somebody else wrote is a binary they built,
	// and this is what they passed. Every verb reads it from here — a verb that
	// reached for forge's own instead would answer about a catalog its caller
	// had not asked for, and the layer they built the binary for would be
	// missing from exactly one stage.
	catalog *plugin.Registry
}

// builtins returns the layers forge itself ships.
//
// Wrapped rather than reached for, so that the one place a catalog is invented
// rather than passed is a named function somebody can find.
...
// Run dispatches one command line against a catalog of layers and returns the
// status to exit with.
//
// The catalog is a parameter because it is the whole of what a run can be told.
// Everything else a command does — which packages to walk, what to write, what
// to say — comes off the command line; which layers exist does not, and cannot,
// because a layer is code that has to be linked in. So a binary that means to
// know about a layer forge does not ship is a binary somebody built, and this
// is what they call.
//
// The streams are parameters for the same reason they always were: a tool whose
// only entry point ends the process can be tested only by starting one.
func Run(catalog *plugin.Registry, args []string, stdout, stderr io.Writer) int {
	env := &environment{
		stdout: stdout, stderr: stderr,
		pipeline: stages(catalog), catalog: catalog,
	}
