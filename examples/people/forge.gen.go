...
func (c *Recent) ReadFrom(r io.Reader) (int64, error) {
	...
	c.AppendSeq(func(yield func(Person) bool) {
		for {
			...
			var v Person
			n, err := decodePeoplePersonJSON(held, 0, 1, &v, false)
			if err != nil {
				failed = err
				return
			}
			if err := jsonAtEnd(held, n); err != nil {
				failed = err
				return
			}
			if !yield(v) {
...
func (c *Credentials) ReadFrom(r io.Reader) (int64, error) {
	...
	c.AppendSeq(func(yield func(Credential) bool) {
		for {
			...
			var v Credential
			n, err := decodePeopleCredentialJSON(held, 0, 1, &v, false)
			if err != nil {
				failed = err
				return
			}
	...
	for first := true; ; first = false {
		...
		case "Aliases":
			...
			if next1, ok := jsonScanNull(b, i); ok {
				...
				out := v.Aliases[:0]
				for first := true; ; first = false {
					next1, done, err := jsonElementNext(b, i, first)
					if err != nil {
						return 0, err
					}
					if done {
